...
namespace CA_Final_Regia.Services.Services.PictureServices
{
    public class PictureResizeService : IPictureResizeService
    {
        public async Task<byte[]> ResizePictureAsync(PictureDto file)
        {
            try
            {
                ...
                using Bitmap bitmap = new(streamForThumbnail);
                Size thumbnailSize = GetThumbnailSize(bitmap);
                using var thumbnail = bitmap.GetThumbnailImage(thumbnailSize.Width, thumbnailSize.Height, null, nint.Zero);
